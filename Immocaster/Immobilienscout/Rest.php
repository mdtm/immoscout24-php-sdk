...
class Immocaster_Immobilienscout_Rest extends Immocaster_Immobilienscout
{
	...
	private function _exportObjectAttachment($aArgs)
	{
		...
		if (!isset($aArgs['title'])) {
			$aArgs['title'] = '';
		}
		if (!isset($aArgs['floorplan'])) {
			$aArgs['floorplan'] = 'false';
		}
		if (!isset($aArgs['titlePicture'])) {
			$aArgs['titlePicture'] = 'false';
		}
		if (!isset($aArgs['type'])) {
			$aArgs['type'] = '';
		}
		if (!isset($aArgs['externalId'])) {
			$aArgs['externalId'] = '';
		}
		if (!isset($aArgs['externalCheckSum'])) {
			$aArgs['externalCheckSum'] = '';
		}
		$oToken = null;
		$sSecret = null;
		$aHeader = array();
	...
	public function getAccess($aArgs)
	{
		try {
			...
			if ($aArgs['verifyApplication'] != true) {
				return false;
			}
		} catch (Exception $e) {
			echo $e->getMessage();
